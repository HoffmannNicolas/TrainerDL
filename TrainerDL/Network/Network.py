...
class MultiLayerPerceptron(torch.nn.Module):
    ...
    def __init__(self, n_inputs : int = 32, n_outputs : int = 64, n_hidden : int = 3, shape : float = 1.0, dropoutRate : float = 0.75):
        ...
        for i in range(n_hidden) :
            networkProportion = (1 + i) / (n_hidden + 2) # 0 for the input layer, 1 for output layer, and in [0, 1] for other hidden self.layers
            ...
            if (networkProportion < 0.5) : # First half of the network : Linear interpolation between <n_inputs> and <n_middle>
                ...
                layerWidth = (n_outputs * middleDistance + n_middle * networkProportion) * 2
